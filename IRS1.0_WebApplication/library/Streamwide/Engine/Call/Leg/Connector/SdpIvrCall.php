...
/**
 * Links a (dead) media server call leg to a sip call leg. Use it only if the initial CREATE signal
 * has a non-empty SDP parameter.
 */
class Streamwide_Engine_Call_Leg_Connector_SdpIvrCall extends Streamwide_Engine_Call_Leg_Connector
{

    /**
     * Error codes
     */
    const CREATE_SIGNAL_SEND_ERR_CODE = 'SDPIVRCALL-100';
    const PROGRESS_SIGNAL_SEND_ERR_CODE = 'SDPIVRCALL-101';
    const FAIL_SIGNAL_SEND_ERR_CODE = 'SDPIVRCALL-102';
    const OK_SIGNAL_RELAY_ERR_CODE = 'SDPIVRCALL-103';
    const FAIL_SIGNAL_RELAY_ERR_CODE = 'SDPIVRCALL-104';
    const MS_NOT_CREATED_ERR_CODE = 'SDPIVRCALL-200';
    const CALL_LEG_DIED_ERR_CODE = 'SDPIVRCALL-201';
    ...
    protected $_errors = array(
        self::CREATE_SIGNAL_SEND_ERR_CODE => 'Unable to send the CREATE signal to SW Engine',
        self::PROGRESS_SIGNAL_SEND_ERR_CODE => 'Unable to send the PROGRESS signal to SW Engine',
        self::FAIL_SIGNAL_SEND_ERR_CODE => 'Unable to send the FAIL signal to SW Engine',
    ...
    {
        // Create the parameters for the CREATE signal
        $params = array();
        ...
        // send the CREATE signal to SW Engine
    ...
    public function setLeftCallLeg( Streamwide_Engine_Call_Leg_Abstract $leftCallLeg )
    {
        ...
        if ( $leftCallLeg->hasSentOrReceivedOk() ) {
            throw new InvalidArgumentException( __METHOD__ . ' requires parameter 1 the be an alive SIP call leg that has not received the OK signal' );
        }
    ...
    /**
     * @see Engine/Call/Leg/Streamwide_Engine_Call_Leg_Connexant#_validateConnectionParams()
     */
    protected function _validateConnectionParams( Array $connectionParams )
    {
        // We need the "sdp" for to perform the connection
        if ( !array_key_exists( 'sdp', $connectionParams ) ) {
            throw new RuntimeException( 'Required "sdp" parameter not provided' );
        }
    ...
    /**
     * We have received the OK signal from the SW Engine, we need to unsubscribe from
     * the other signals, set the MS call as alive and relay the OK signal to the SIP
     * call leg and dispatch the CONNECTED event
     *
    ...
    {
        // unsubscribe from the OK and FAIL signals
