 ...
 * that has a Streamwide_Engine_Signal parameter that, in turn, has a certain name (CHILD, ARM, etc)
 */
class Streamwide_Engine_NotifyFilter_Factory_EventSignalParameterName extends Streamwide_Engine_NotifyFilter_Factory
{
     ...
     *
     * This filter doesn't make sense (a signal name will never be an instance of any class, unless PHP will introduce a String class)
     * but its here to conform with the filter factory API
     */
    ...
    protected function _createFilter( $filter )
    {
        return new Streamwide_Engine_Events_Event_Param_Signal_Specification_Decorator(
            new Streamwide_Engine_Signal_Name_Specification_Decorator(
                $filter
            )
        );
    }
