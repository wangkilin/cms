...
/*
	...
	$Id: admin_check.php 11150 2009-02-20 01:35:59Z zhaofei $
*/
...
if(submitcheck('listsubmit')) {
	...
	switch ($_POST['operation']) {
		case 'check':
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET grade=\''.intval($_POST['opgrade']).'\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'ban':
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET grade=\'-1\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'reply':
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET allowreply=\''.intval($_POST['allowreply']).'\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'delete':
			//删除html
			deleteitemhtml($itemidarr);
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET folder=\'3\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'movecat':
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET catid=\''.intval($_POST['opcatid']).'\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'republish':
			$_SGLOBAL['db']->query('UPDATE '.tname('spaceitems').' SET folder=\'1\' WHERE itemid IN (\''.$itemidstr.'\')');
			break;
		case 'remove':
...
if (empty($_GET['op']) && ($status == 'no' || $status == 'yes' || $status == 'delete' || $status == 'ban')) {
	...
	if($status == 'delete') {
		$wheresqlarr[] = 'i.folder=3';
	} else {
		$wheresqlarr[] = 'i.folder=1';
		if($status == 'yes') {
			if(empty($grade)) {
	...
	$multipage = '';
	if($listcount) {
		$query = $_SGLOBAL['db']->query('SELECT i.*, f.name FROM '.tname('spaceitems').' i LEFT JOIN '.tname('categories').' f ON f.catid=i.catid WHERE '.$wheresqlstr.' ORDER BY i.dateline DESC LIMIT '.$start.','.$perpage);
...
//帮助
if(empty($_GET['op']) && $status == 'help') {
	echo label(array('type'=>'table-start'));
	echo label(array('type'=>'text', 'alang'=>'check_explain_title', 'text'=>$alang['check_explain']));
	echo label(array('type'=>'table-end'));
}
