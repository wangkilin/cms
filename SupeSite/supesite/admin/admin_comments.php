...
/*
	...
	$Id: admin_comments.php 11552 2009-03-10 05:21:35Z zhaolei $
*/
...
if(submitcheck('searchsubmit') || !empty($search)) {
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		if(empty($value['url'])) {
			...
			if(empty($itemarr2[$value['itemid']])) $itemarr2[$value['itemid']] = 0;
			$itemarr2[$value['itemid']]++;
		}
	}
	$_SGLOBAL['db']->query("DELETE FROM ".tname('spacecomments')." WHERE cid IN ('$itemids')");
	
	foreach ($itemarr as $itemid => $num) {
		$num2 = empty($itemarr2[$itemid])?0:$itemarr2[$itemid];
