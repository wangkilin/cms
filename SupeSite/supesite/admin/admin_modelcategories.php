...
/*
	...
	$Id: admin_modelcategories.php 11221 2009-02-26 02:57:03Z zhaolei $
*/
...
//х╗оч
if(!checkperm('managemodelcategories')) {
...
if(submitcheck('listsubmit')) {
	...
	if(is_array($_POST['displayorder']) && !empty($_POST['displayorder'])) {
		foreach ($_POST['displayorder'] as $postcatid => $postdisplayorder) {
			updatetable($resultmodels['modelname'].'categories', array('displayorder'=>$postdisplayorder), array('catid' => $postcatid));
		}
	}
	...
	if($_POST['catid'] > 0) {
		...
		updatetable($resultmodels['modelname'].'categories', $setsqlarr, array('catid' => $_POST['catid']));

		updatemodel('modelname', $resultmodels['modelname']);
		showmessage('category_update_success', $newurl);
	} else {
		$catid = inserttable($resultmodels['modelname'].'categories', $setsqlarr, 1);
		$subcatidarr[] = $catid;
		$subcatidarr = array_unique($subcatidarr);
		$subcatid = implode(',', $subcatidarr);

		$_SGLOBAL['db']->query('UPDATE '.tname($resultmodels['modelname'].'categories').' SET subcatid=\''.$subcatid.'\' WHERE catid=\''.$catid.'\'');
	...
	if(empty($newcatid)) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$ids[] = $value['itemid'];
			inserttable($resultmodels['modelname'].'folders', $setsqlarr);
		}
		$ids = simplode($ids);
		...
		$_SGLOBAL['db']->query('DELETE FROM '.tname($resultmodels['modelname'].'categories').' WHERE catid=\''.$catid.'\'');
	} else {
		$_SGLOBAL['db']->query('DELETE FROM '.tname($resultmodels['modelname'].'categories').' WHERE catid=\''.$catid.'\'');
...
if (empty($_GET['op'])) {
	...
	//ONE VIEW FOR UPDATE
	$query = $_SGLOBAL['db']->query('SELECT * FROM '.tname($resultmodels['modelname'].'categories').' WHERE catid=\''.$_GET['catid'].'\'');
	...
	$_GET['catid'] = intval($_GET['catid']);
	$query = $_SGLOBAL['db']->query('SELECT catid FROM '.tname($resultmodels['modelname'].'categories').' WHERE upid=\''.$_GET['catid'].'\'');
