...
/*
	...
	$Id: admin_modelmanages.php 11948 2009-04-15 06:38:46Z zhaofei $
*/
...
//权限
if(!checkperm('managemodelmanages') && !empty($allowlogin)) {
...
//POST METHOD
if (submitcheck('listvaluesubmit') && !empty($allowlogin)) {
...
if (empty($_GET['op'])) {
	if(empty($allowlogin)) {
		showmessage('', CPURL.'?action=modelmanages&op=add&mid='.$resultmodels['mid'], 0);
	}
	if(empty($showurlarr)) {
		//CATEGORY
...
} elseif (!empty($allowlogin) && ($_GET['op'] == 'edit' || $_GET['op'] == 'view')) {
	$itemid = intval($_GET['itemid']);
	$sqlplus = '';
	if(!empty($itemid)) {
		$wheresqlstr = getwheresql($wheresqlarr);
		if(!empty($_GET['folder']) && $_GET['op'] == 'view') {
			$query = $_SGLOBAL['db']->query('SELECT * FROM '.tname($resultmodels['modelname'].'folders').' WHERE itemid=\''.$itemid.'\' AND '.$wheresqlstr);
	...
	<tr>
		...
		<td class="actions">
			<table summary="" cellpadding="0" cellspacing="0" border="0" align="right">
				<tr>
';
if(!empty($allowlogin)) echo '<td'.$viewclass.'><a href="'.$theurl.'" class="view">'.$alang['spaces_spacecp'].'</a></td>';
