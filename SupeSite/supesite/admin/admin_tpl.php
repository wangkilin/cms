...
/*
	...
	$Id: admin_tpl.php 11659 2009-03-17 08:39:03Z zhaolei $
*/
...
if(submitcheck('valuesubmit')) {
	...
	if(!empty($filename)) {
		$filefullname = S_ROOT.'./templates/'.$_SCONFIG['template'].'/'.$filename.'.html.php';
...
if (empty($_GET['op'])) {
	//LIST VIEW
	$tpldir = dir(S_ROOT.'./templates/'.$_SCONFIG['template']);
	$tplarray = array();
...
<table summary="" id="pagehead" cellpadding="0" cellspacing="0" border="0" width="100%">
	<tr>
		<td><h1>'.$alang['admincp_header_tpl_list'].'</h1></td>
		<td class="actions">
			<table summary="" cellpadding="0" cellspacing="0" border="0" align="right">
				<tr>
					<td'.$viewclass.'><a href="'.$theurl.'" class="view">'.$alang['tpl_view_list'].'</a></td>
					<td'.$addclass.'><a href="'.$theurl.'&op=add" class="add">'.$alang['tpl_add'].'</a></td>
				</tr>
			</table>
		</td>
	</tr>
...
if($listarr) {
	...
	echo '<tr><th style="text-align: left;">'.$alang['admincp_header_tpl_list'].' ( '.$alang['style_dir'].' ./templates/'.$_SCONFIG['template'].'/ )</th></tr>';
...
if($thevalue) {
	...
	echo '<tr><th style="text-align: left;">templates/'.$_SCONFIG['template'].'/<strong><i>'.$thevalue['filename'].'</i></strong>.html.php</th></tr>';
...
if($addvalue) {
	...
	echo label(array('type'=>'text', 'alang'=>'tpl_title_filename', 'text'=>'templates/'.$_SCONFIG['template'].'/<input type="text" name="filename" value="'.$addvalue['filename'].'" />'.'.html.php'));
	echo label(array('type'=>'table-end'));
	echo label(array('type'=>'div-end'));
