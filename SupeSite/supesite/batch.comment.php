...
/*
	...
	$Id: batch.comment.php 11948 2009-04-15 06:38:46Z zhaofei $
*/
...
if($action == 'rate') {
	...
	$item = array();
	if(!empty($itemid)) {
		$query = $_SGLOBAL['db']->query("SELECT i.uid, i.type, i.itemid FROM ".tname('spaceitems')." i WHERE i.itemid='$itemid' AND i.folder=1 AND i.allowreply=1 AND i.tid=0");
