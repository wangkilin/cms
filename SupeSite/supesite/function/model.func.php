...
/*
	...
	$Id: model.func.php 11970 2009-04-22 06:12:52Z zhaofei $
*/
...
function deletemodelitems($modelname, $ids, $mid=0, $undel=0, $folder=2) {
	...
	if($undel) {	//移动到废件箱
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			inserttable($modelname.'folders', $setsqlarr);
		}
		...
		if(!empty($mid)) {
			...
			$_SGLOBAL['db']->query("DELETE FROM ".tname($modelname.'items')." WHERE itemid IN ($ids)");
			$_SGLOBAL['db']->query("DELETE FROM ".tname($modelname.'message')." WHERE itemid IN ($ids)");
			$_SGLOBAL['db']->query("DELETE FROM ".tname($modelname.'comments')." WHERE itemid IN ($ids)");
			$_SGLOBAL['db']->query("DELETE FROM ".tname($modelname.'rates ')." WHERE itemid IN ($ids)");
			return true;
...
function changemodelfolder($modelname, $ids, $undel=0) {
	...
	if($undel == 1) {	//还原
		...
		$query = $_SGLOBAL['db']->query('SELECT * FROM '.tname($modelname.'folders').' i WHERE i.itemid IN ('.$ids.')');
		$i = $j = 0;
		$addfeed = array();
		while ($result = $_SGLOBAL['db']->fetch_array($query)) {
		...
		$_SGLOBAL['db']->query("DELETE FROM ".tname($modelname.'folders')." WHERE itemid IN ($ids)");

	} elseif($undel == 2) {
		$_SGLOBAL['db']->query('UPDATE '.tname($modelname.'folders').' SET folder=2 WHERE itemid IN ('.$ids.')');
	} elseif($undel == 3) {
		$idarr = $itemid = array();
		$query = $_SGLOBAL['db']->query('SELECT * FROM '.tname($modelname.'folders').' i WHERE i.itemid IN ('.$ids.')');
		while ($result = $_SGLOBAL['db']->fetch_array($query)) {
...
function modelpost($cacheinfo) {
	...
	$_POST['allowreply'] = empty($_POST['allowreply']) ? 0 : !empty($modelsinfoarr['allowcomment']) ? 1 : 0;
	...
	if(checkperm('managemodelfolders')) {
	...
	$modelsinfoarr['allowgrade'] = checkperm('managemodelfolders') ? 0 : (checkperm('modelneedcheck') ? 1 : $modelsinfoarr['allowgrade']);
	...
	if(empty($modelsinfoarr['allowgrade']) || (!empty($modelsinfoarr['allowgrade']) && !empty($itemid))) {	//不需要审核时入item表
		if(empty($itemid)) {
			//插入数据
	...
	//添加内容
	if(!empty($modelsinfoarr['allowfilter'])) $setsqlarr = scensor($setsqlarr, 1);
	if(empty($modelsinfoarr['allowgrade']) || (!empty($modelsinfoarr['allowgrade']) && $op == 'update')) {	//不需要审核时入message表
		if($op == 'add') {
		...
		if(!empty($modelsinfoarr['allowgrade']) && $op == 'update') {
			deletemodelitems($modelsinfoarr['modelname'], array($itemid), $_POST['mid'], 1, 1);
		}

		if(!empty($modelsinfoarr['allowgrade']) && $op == 'update') {
			showmessage('writing_success_online_please_wait_for_audit', empty($setsqlarr['uid']) ? S_URL."/admincp.php?action=modelmanages&op=add&mid=$modelsinfoarr[mid]" : S_URL.'/'.$theurl.'&mid='.$modelsinfoarr['mid']);
		} else {
			showmessage('online_contributions_success', S_URL.'/'.$theurl.'&mid='.$modelsinfoarr['mid']);
		}
	} else {
		...
		$itemid = inserttable($modelsinfoarr['modelname'].'folders', $setsqlarr, 1);
		...
		showmessage('writing_success_online_please_wait_for_audit', empty($setsqlarr['uid']) ? S_URL."/admincp.php?action=modelmanages&op=add&mid=$modelsinfoarr[mid]" : S_URL."/admincp.php?action=modelfolders&mid=$modelsinfoarr[mid]");
	}
}
