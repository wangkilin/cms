...
function getdata(formbody, posturl, robotid) {
	...
	if(poststr == '') {
		...
		if(robotid != 0) {
			obj_message.innerHTML = '<a href="' + siteUrl + '/admincp.php?action=robotmessages&robotid='+robotid+'">采集完毕，点击查看采集结果</a>';
		} else {
	...
	xmlhttp.onreadystatechange = function() {
		if(xmlhttp.readyState==4) {
			...
			if(xmlhttp.status==200){
				...
				re = re.split('!|!');
				obj_ext_return.innerHTML = '<font style="color: #0F0">采集成功</font> <a href="' + siteUrl + '/admincp.php?action=robotmessages&op=viewmessage&itemid='+re['1']+'" target="_blank">查看</a>';
			} else {
