...
?>
	...
	<p>本程序用于升级 UCenter 1.0beta 到 UCenter 1.0</p>
	<p>运行本升级程序之前，请确认已经上传 UCenter 1.0 的全部文件和目录</p>
	<p>强烈建议您升级之前备份数据库资料</p>
	<p><a href="<?php echo $PHP_SELF;?>?action=updatepw">如果您已确认完成上面的步骤,请点这里升级</a></p>
...
<?php
	...
	//note 处理 config.inc.php

	define('UC_CONFIG', UC_ROOT.'./data/config.inc.php');
	if(!is_writable(UC_CONFIG)) {
		showmessage('配置文件 ./data/config.inc.php 不可写。(*nix系统请设置文件权限为 777，Windows系统设置成II帐户为可写)。');
	}
	if(empty($_POST['password'])) {
		showheader();
		echo '<br /><br /><h3>升级程序需要重置创始人密码，账号为系统内置，请牢记密码。</h3>';
		echo '<form action="upgrade1.php?action=updatepw" method="post">';
		echo '创始人账号：<input type="text" name="founder" disabled="disabled" value="UCenterAdministrator" /><br />';
		echo '创始人密码：<input type="password" name="password" /><br />';
		echo '　重复密码：<input type="password" name="password2" /><br />';
