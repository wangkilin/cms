...
class pgsql
{
	...
	public function pgsql($dbSeq=0)
	{
		 ...
		 include_once(MY_ROOT_PATH."language/class/".(isset($_system)?$_system['config']['default_language']:MY_DEFAULT_LANGUAGE)."/pgsql.lang.php");//load class language
		 $this->connect($MY_db_config['pgsql'][$dbSeq]);
	}// END::function pgsql
	...
	private function connect($connArray)
	{
		...
		if(is_resource($conn))
		{
			...
			$this->_errorStr= __CLASS__."->".__FUNCTION__."()".$MY_lang->classModule['pgsql']['failConnection'];
	...
	public function query($query)
	{
		...
		{
			$this->_errorStr= __CLASS__."->".__FUNCTION__."()".$MY_lang->classModule['pgsql']['failQuery'].$query;
			return false;
		}
		...
		if($this->debug)
			echo (__CLASS__."->".__FUNCTION__."() ".$MY_lang->classModule['pgsql']['commandDone']."<br> \n");
	...
	public function close()
	{
		...
		elseif($this->cacheError)
			$this->_errorStr= __CLASS__."->".__FUNCTION__."()".$MY_lang->classModule['pgsql']['failCloseDb'];
		if($this->debug)
			echo (__CLASS__."->".__FUNCTION__."() ".$MY_lang->classModule['pgsql']['commandDone']."<br> \n");
	}//END::function close
