...
/********************************************************/
/**
 *@FileName : modules/news/class/module_media_front.class.php
 *@Author	: WangKilin
...
class My_Media_Module extends module
{
    ...
    public function __construct($moduleInfo)
    {
        ...
        //$this->_mediaModel = $My_Kernel->getClass('news', $My_Kernel->getSystemDbHandler());
        $this->_loadModel($My_Kernel->getSystemDbHandler());
    }
    protected function _loadModel($db)
    {
        $this->_mediaModel = new My_News($db);
    }
    ...
    public function  returnWeb ()
    {
        ...
        if (isset($this->_params['type'])) {// show block
            switch (strtolower($this->_params['type'])) {
                ...
                default: // show specified media categories
    ...
    public function displayNews($options = array())
    {
        ...
        if(isset($this->_params['news_id'])) {
            if($news = $this->_mediaModel->getNewsById($this->_params['news_id'])) {
                $newsList = array($news);
            };
        } else if (isset($this->_params['news_ids'])) {
            $newsList = $this->_mediaModel->getNewsByIds($this->_params['news_ids']);
            ...
            switch($this->_params['newsType']) {
                case 'latest':
                    ...
                    $newsList = $this->_mediaModel->getLatestNews($randomLimit);
                    break;
                ...
                default:
                    ...
                    $newsList = $this->_mediaModel->getRandomNews($randomLimit, $categoryId);
            ...
            if(isset($_REQUEST['news_id'])) {
                $news = $this->_mediaModel->getNewsById($_REQUEST['news_id']);
                $_categoryId = $news['category_id'];
                $newsList = $this->_mediaModel->getNewsByCategoryId($_categoryId);
                $options = array(
                ...
                $newsList = $this->_mediaModel->getNewsCategories();
                $action = 'showNewsCategories';
            } else {
                $newsList = $this->_mediaModel->getRandomNews(5);
                $options = array(
                'showCreateTime'   => false,
                'showCreateUser'   => false,
                'showReferenceUrl' => false,
    ...
    public function displayCategory($options=array())
    {
        ...
        if(isset($_REQUEST['category_id'])) {
            $items = $this->_mediaModel->getNewsByCategoryId($_REQUEST['category_id']);
            $totalNumber = $this->_mediaModel->countNewsByCategoryId($_REQUEST['category_id']);
            $listKeys = array(array('key' => '<a href="?yemodule='.$this->_moduleInfo['moduleName'].'&news_id=%d" >%s</a>',
                                'value' => array('news_id', 'title')),
                    );
        } else {
            $items = $this->_mediaModel->getNewsCategories();
