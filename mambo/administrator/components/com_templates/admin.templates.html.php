<?php
/**
* @version $Id: admin.templates.html.php,v 1.2 2004/10/11 03:36:33 dappa Exp $
* @package Mambo_4.5.1
...
class HTML_templates {
	...
	function showTemplates( &$rows, &$pageNav, $option, $client ) {
		...
		function showInfo(name) {
			...
			if (document.adminForm.doPreview.checked) {
				...
				var html=name;
				html = '<br /><img border="1" src="'+src+'" name="imagelib" alt="<?php echo $adminLanguage->A_COMP_TEMP_NO_PREVIEW;?>" width="206" height="145" />';
	...
	*/
	function editTemplateSource( $template, &$content, $option, $client ) {
		...
		<tr>
			<th class="templates">
			<?php echo $adminLanguage->A_COMP_TEMP_EDITOR;?>
			</th>
		</tr>
		</table>
		...
		<tr>
			<th colspan="4">
			<?php echo $adminLanguage->A_COMP_TEMP_PATH;?>/<?php echo $template; ?>/index.php
      		<?php
      		$template_path = $mosConfig_absolute_path . ( $client == 'admin' ? '/administrator' : '' )
      		. '/templates/' . $template . '/index.php';
      		echo is_writable( $template_path ) ? '<b><font color="green">'. $adminLanguage->A_COMP_TEMP_WRT .'</font></b>' : '<b><font color="red">'. $adminLanguage->A_COMP_TEMP_UNWRT .'</font></b>';
      		?>
			</th>
		</tr>
		<tr>
	...
	*/
	function editCSSSource( $template, &$content, $option, $client ) {
		...
		<tr>
			<th class="templates">
			<?php echo $adminLanguage->A_COMP_TEMP_ST_EDITOR;?>
			</th>
		</tr>
		</table>
		...
		<tr>
			<th colspan="4">
			<?php echo $adminLanguage->A_COMP_TEMP_NAME;?>: <?php echo $client == 'admin' ? 'administrator/' : ''; ?>templates/<?php echo $template; ?>/css/template_css.php
			<?php
			$css_path = $mosConfig_absolute_path . ($client == 'admin' ? '/administrator' : '')
			. '/templates/' . $template . (!$adminLanguage->RTLsupport ? '/css/template_css.css' : '/css/template_css_rtl.css'); /* rtl change */
			echo is_writable( $css_path ) ? '<b><font color="green">'. $adminLanguage->A_COMP_TEMP_WRT .'</font></b>' : '<b><font color="red">'. $adminLanguage->A_COMP_TEMP_UNWRT .'</font></b>';
      		?>
			</th>
		</tr>
		<tr>
	...
	*/
	function assignTemplate( $template, &$menulist, $option ) {
		...
		<tr>
			...
			<?php echo $adminLanguage->A_COMP_TEMP_ASSIGN_TP;?> <?php echo $template; ?>
            <?php echo $adminLanguage->A_COMP_TEMP_TO_MENU;?>
			</th>
		</tr>
		<tr>
