<?php
/**
* @version $Id: contact.html.php,v 1.35 2004/09/26 10:37:23 stingrey Exp $
* @package Mambo_4.5.1
...
class HTML_contact {
	...
	*/
	...
	function viewcontact( &$contact, &$params, $count, &$list ) {
		//print_r( $contact );
		...
		if ( $contact->name ||  $contact->con_position ) {
			if ( $contact->name && $params->get( 'name' ) ) {
				...
				<tr>
					...
					<?php
					// displays Print Icon
					$print_link = $mosConfig_live_site. "/index2.php?option=com_contact&amp;Itemid=". $Itemid ."&amp;contact_id=". $contact->id ."&amp;pop=1";
