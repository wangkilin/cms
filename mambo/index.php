...
$option = trim( strtolower( mosGetParam( $_REQUEST, 'option' ) ) );//得到$option=$_REQUEST['option'];
$Itemid = intval( mosGetParam( $_REQUEST, 'Itemid', null ) );//得到$Itemid=$_REQUEST['Itemid'];
...
if ($option == '') {
	if ($Itemid) {
		...
		. "\n AND published = '1'"
		;
		$database->setQuery( $query );//替换掉#__;
	} else {
		$query = "SELECT id, link"
		. "\n FROM #__menu"
