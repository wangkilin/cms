...
define('ROOT_PATH', dirname(__FILE__) . '/ad_sender');
set_include_path(ROOT_PATH . PATH_SEPARATOR . get_include_path());

if (!is_file(ROOT_PATH . '/inc/emailAdConfig.ini')) { // config file not ready
    exit;
}

$config = @parse_ini_file( ROOT_PATH . '/inc/emailAdConfig.ini', true);
...
require(ROOT_PATH . '/inc/class.phpmailer.php'); // include php mailer class
...
function sendEmail($emailList)
{
    ...
    $mail->IsSMTP();
    $mail->PluginDir = ROOT_PATH . '/inc/';
    $smtpList = array(1, 3);
    $rand = rand(0, 1);
    $config['GLOBAL_CONFIG']['WhichSmtpConfigToBeUsed'] = $smtpList[$rand];
    if (! $config['GLOBAL_CONFIG']['WhichSmtpConfigToBeUsed']) {
        exit;
    }
    $smtpConfig = $config['SMTP_' . $config['GLOBAL_CONFIG']['WhichSmtpConfigToBeUsed'] . '_CONFIG'];
    $mail->Host = $smtpConfig['SmtpHost']; // SMTP servers
    echo "var smtpServer = ' using {$smtpConfig['SmtpHost']} to send ad';\n";
